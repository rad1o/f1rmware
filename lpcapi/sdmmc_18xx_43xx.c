...
// we lie here so the value calculated doesn't exceed 255. Should still be
// exact enough (~0.5% off)
#define CLK_SPEED 203000000
...
static int32_t sdmmc_execute_command(LPC_SDMMC_T *pSDMMC, uint32_t cmd, uint32_t arg, uint32_t wait_status)
{
	...
	while (step) {
		Chip_SDIF_SetClock(pSDMMC, CLK_SPEED, g_card_info->card_info.speed);
