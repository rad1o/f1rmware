/* music.c */
/***************************************************************************
 * MOD Player for the Rad1o
 *
 * MOD player parts from scratch by Rainer Sinsch
...
static struct s_song modsong;               /* The Song */
static struct s_modplayer modplayer;        /* The Module Player */
static struct s_mixer mixer;

/* The Amiga Period Table (+1 because we use index 0 for period 0 = no new note) */
static unsigned short s_periodtable[37*8+1] = 
...
/* The sin table */
static signed short s_sintable[0x40] = 
...
static void mixer_playsample(int channel, int instrument);
...
bool loadmod(void *modfile)
{
    ...
    periodsconverted = (unsigned char*)modfile + 20 + modsong.noofinstruments*30 + 1; 
...
static void vibrate(int channel);
...
static void tremolo(int channel);
...
static void slidetonote(int channel);
...
static void slidetonoteglissando(int channel);
...
/* Apply Volume Slide */
static void volumeslide(int channel, int effectx, int effecty);
...
static void playline(int pattern, int line);
...
static void playeffect(int currenttick);
...
void modplay(char *fname) {
    ...
    f_read(&file, modfile, MODFILE_BUF_SIZE, &readbytes);
    f_close(&file);
...
void select_mod(void) {
    ...
    for(;;) {
        getInputWaitRelease();
        if(selectFile(filename,"MOD")<0){
            break;
        }

        lcdPrintln("Now playing:");
        lcdPrintln(filename);
        lcdPrintln("Up/Down: Vol");
        lcdPrintln("Left: Select");
        lcdDisplay();
        modplay(filename);
    }
    return;
